...
function Leaderboard(): JSX.Element {
    ...
    const list = leaderboard.map((leaderboardPosition: string[], index: number) =>
        <ul className={"opacity-95 hover:bg-indigo-400 p-2 select-none hover:text-white duration-200 text-white text-center w-full text-center text-2xl"} key={index}>{index + 1}. {leaderboardPosition[0]} ({leaderboardPosition[1]})</ul>
    );
    ...
    return (
            <div className={"select-none flex justify-center items-center h-screen min-h-fit w-screen pattern-rhombus pattern-sky-50 pattern-bg-indigo-400 pattern-size-8 pattern-opacity-40"}>
                <div className={"py-0.5 scrollbar-hide overflow-y-scroll border-4 border-violet-200 flex-col bg-indigo-500 h-4/6 w-11/12 md:w-3/5 xl:w-2/5 rounded-2xl opacity-95"}>
